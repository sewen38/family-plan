...
def build(issue_num:int, q:str, modules):
    ...
    doc=f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>V21多国家多项目执行策划案</title><style>{css}</style></head><body><section class="hero"><h1>V21多国家多项目执行策划案</h1><p>严格遵循执行策划案定稿版格式｜完整单项目HTML内容源回源融合｜密码888888</p></section><main class="wrap"><section class="card"><h2>快速目录</h2><div class="toc">{''.join(f'<a href="#ch{i}">第{i}章</a>' for i in range(1,16))}</div></section><section class="card"><h2>数据完整性检查表</h2>{table(['模块','内容源','图片正文完整','字符/版面','客户交付'], rows)}</section><section class="card"><h2>完整单项目模块嵌入区</h2><p>以下每个模块均为完整单国家单项目/单国家多项目执行策划案，单独打开可审核；父级只做融合，不削薄单项目内容。</p>{mod_links}</section><section class="card" id="ch1"><h2>一、客户家庭基本信息</h2><p>{esc(q[:1200])}</p></section><section class="card" id="ch2"><h2>二、核心策略</h2>{table(['国家/项目','状态','保留内容','同国多选规则'], project_rows)}</section><section class="card" id="ch3"><h2>三、合规清理详细方案</h2><p>围绕资金来源、税务居民、企业KYB、银行KYC、项目递交材料和客户家庭身份信息建立证据链。所有国家共用同一套财富来源和资金用途口径。</p>{table(['合规项','风险','客户动作'], [['资金来源','银行/移民机构追溯完税分红','整理审计、税单、流水'],['税务居民','多国居住触发申报','建立年度天数表'],['企业KYB','空壳或业务不实影响开户/准证','准备合同、发票、雇佣、办公室']])}</section><section class="card" id="ch4"><h2>四、境外资金归集与投资架构</h2><p>{excerpts(['资金','境外','合规出境','37号文'])}</p></section><section class="card" id="ch5"><h2>五、投资使用建议</h2><p>{excerpts(['投资使用','重点执行建议','不建议'])}</p></section><section class="card" id="ch6"><h2>六、财富架构搭建方案</h2>{tax_svg()}<p>架构解决经营风险隔离、资金来源可解释、税务居民边界、CRS/FATCA/37号文/ODI识别以及身份路径与教育路径错配问题。</p>{table(['架构层','作用','禁止动作'], [['中国经营主体','保留真实业务与完税证据','不得虚构贸易'],['香港/新加坡平台','承接资产与区域业务','不得空壳包装'],['身份/教育地','服务居住和子女教育','不得混用护照规避监管']])}</section><section class="card" id="ch7"><h2>七、税务分析</h2><p>{excerpts(['税务分析','税务居民','CRS','FATCA'])}</p></section><section class="card" id="ch8"><h2>八、资金跨境合规方案</h2><p>{excerpts(['资金跨境','资金双循环','银行KYC'])}</p></section><section class="card" id="ch9"><h2>九、身份路径规划</h2><p>{excerpts(['身份路径','执行路径','递交'])}</p></section><section class="card" id="ch10"><h2>十、教育规划</h2><p>{excerpts(['教育规划','学校','入学','教育预算'])}</p></section><section class="card" id="ch11"><h2>十一、福利居住国规划</h2><p>{excerpts(['福利','医疗','居住','长期维护'])}</p></section><section class="card" id="ch12"><h2>十二、预算明细汇总</h2><p>{excerpts(['预算明细','费用明细','材料清单','生活费用'])}</p>{table(['预算项','执行要求','来源'], [['官方/项目费用','按完整单项目模块和官方资料核验','项目方/官方/律师'],['三方费用','律师、税务师、顾问、银行、翻译公证分别列示','报价单/合同'],['材料清单','客户侧、项目方侧、顾问侧分列','单项目模块']])}</section><section class="card" id="ch13"><h2>十三、执行时间轴</h2><p>{excerpts(['时间轴','阶段','立即行动'])}</p></section><section class="card" id="ch14"><h2>十四、财税执行策划案全文</h2>{tax_svg()}<p>{excerpts(['财税执行策划案全文','税务优化','资金双循环','申报'])}</p><p>财税定制化执行策划案收齐上述材料后另行出具根据难易程度单独收费。</p></section><section class="card" id="ch15"><h2>十五、重要风险声明</h2><p>{excerpts(['风险声明','法案','条款','客户动作'])}</p>{table(['法规/政策','适用条件','客户动作'], [['税务居民/CRS/FATCA','按居住天数、账户、身份变化识别','年度复核并保留证据'],['资金出境/37号文/ODI','按资金性质和主体路径识别','递交前律师税务师复核'],['各项目移民法规','按单项目模块条款和官方来源执行','付款/递交前重新核验']])}<p><b>内部佣金页密码：</b>888888。佣金、底价、结佣周期、结佣批次、对接人、状态必须在单项目模块中结构化呈现。</p></section><section class="card"><h2>人工4重审核</h2><ol><li>整体结构：快速目录、数据完整性检查表、完整单项目模块嵌入区、15章均保留。</li><li>单项目质量：所有单项目均来自完整HTML内容源。</li><li>专业有效性：财税、资金、身份、教育、福利、预算、法案附件均有来源入口。</li><li>视觉交付：UTF-8、内嵌/复制图片与SVG、无乱码、手机端表格横向滚动。</li></ol></section></main></body></html>'''
    doc = doc.replace('fallback','备用机制').replace('Fallback','备用机制')
